    ...
    def __init__(self, *args, region=None, **kwargs):
        ...
        if "invalid" not in self.error_messages:
            if region:
                ...
                error_message = _(
                    "Введіть коректний номер телефону (Приклад: {example_number}) "
                    "or a number with an international call prefix."
                )
            else:
                example_number = "+380630000000"  # Ghostbusters
                # Translators: {example_number} is an international phone number.
                error_message = _("Введіть коректний номер телефону (Приклад: {example_number}).")
